...
Kuntamalla, Srinivas, and L. Ram Gopal Reddy. "An efficient and automatic systolic peak detection algorithm 
for photoplethysmographic signals." International Journal of Computer Applications 97.19 (2014).

'''

def vpd_peakdetection2(data):
    
    # 3-point moving average smoothing
    fwd = moving_average(data, size=5)
    bwd = moving_average(data[::-1], size=5)
    average_data = np.mean(np.vstack((fwd,bwd[::-1])), axis=0)
    
    
    # find all the peaks and valleys    
    peak, peak_idx, valley, valley_idx = [],[],[],[]
    
    for i in range(1, len(average_data)-1):
        
        # peaks: P(n) = S(n): s(n-1)<S(n)>S(n+1)
        if (average_data[i-1] < average_data[i] and average_data[i] > average_data[i+1]):
            peak.append(average_data[i])
            peak_idx.append(i)
            
        # valleys: V(n) = S(n): S(n-1) > v< S(n+1)
        if average_data[i-1] > average_data[i] and average_data[i] < average_data[i+1]:
            valley.append(average_data[i])
            valley_idx.append(i)
     
  
    # if the peak comes first, discard and the signal is taken starting from the valley
    if peak_idx[0] < valley_idx[0]:
        peak = np.delete(peak,0)
        peak_idx = np.delete(peak_idx,0)
            
    if len(peak) < len(valley):
        valley = np.delete(valley,-1)
        valley_idx = np.delete(valley_idx,-1)
     
    if len(peak) > len(valley):
        peak = np.delete(peak,-1)
        peak_idx = np.delete(peak_idx,-1)
            
    # repeat until the number of peaks in the two successive iterations remains the same
    numOfIteration = 0
    numOfPeaks = len(peak) # number of peaks
    
    while numOfIteration < 2:
        # calculate the interval VPD(k) = P(k) - V(k)
        vpd = []
        for i in range(len(peak)):
            vpd.append(peak[i] - valley[i])
            
        if len(vpd) < 3:
            break
        
        eliminate_list = []
       # print(len(vpd), len(peak), len(peak_idx), len(valley), len(valley_idx))
        for i in range(1, len(vpd)-1):
            if vpd[i] < 0.4 * (vpd[i-1] + vpd[i] + vpd[i+1]) / 3:
                eliminate_list.append(i)
                
        peak = np.delete(peak,eliminate_list)
        peak_idx = np.delete(peak_idx,eliminate_list)
        valley = np.delete(valley,eliminate_list)
        valley_idx = np.delete(valley_idx,eliminate_list)

        numOfIteration += 1

        if numOfPeaks == len(peak):
            numOfIteration += 1
...
for i in range(0,len(dy)-fs*2,fs*2):
    th = np.mean([dy[x] for x in range(i,i+fs*2) ]) * 3
    for j in range(i,i+fs*2,10):
        max_ = np.argmax([dy[x] for x in range(j,j+10)]) + j
       # print(j, max_, dy[max_])
        if dy[max_] >= th:
            if j == 0:
                peak.append(max_)
                bef_max_ = max_
            else:
                if (max_ - bef_max_) > (300 * fs /1000):
                    peak.append(max_)        
                    bef_max_ = max_
